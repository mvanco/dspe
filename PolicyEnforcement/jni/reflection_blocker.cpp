...
void HookReflectionMethods()
{
	...
	for(int i = 0; reflection_hooks[i].ObjectName; i++)
	{
		...
		LOG_TO_CAT("dvmFindClassByName(%s) returns %p.", reflection_hooks[i].ClassName, clazz);
		...
		for (int j=0; hook_methods[j].MethodName; j++)
		{
			...
			for(int k = 0; dvm_methods[k].name; k++)
				...
				&& !strcmp(dvm_methods[k].signature, hook_methods[j].MethodSignature))
				{
					...
					if (*hook_methods[j].OriginalFuncPP && (*hook_methods[j].OriginalFuncPP != dvm_methods[k].fnPtr))
						LOG_TO_CAT("Double-use OriginalFuncPP in %s.", hook_methods[j].MethodName);
					// Store original entry in dvm_methods[k];
					LOG_TO_CAT("Replace %p @ %p with %p", dvm_methods[k].fnPtr, &(dvm_methods[k].fnPtr), hook_methods[j].NewFuncPtr);
					*hook_methods[j].OriginalFuncPP = dvm_methods[k].fnPtr;
					...
					if (clazz)
					{
						...
						if (method)
						{
							LOG_TO_CAT("dvmSetNativeFunc(%p) invoked. %x %x %x", method, *(int*)((char*)method+0x24), *(int*)((char*)method+0x28), *(int*)((char*)method+0x2C));
			...
			if (!hooked)
				...
				LOG_TO_CAT("Hooked %s(%s).", hook_methods[j].MethodName, hook_methods[j].MethodSignature);
		}
	}
...
//	LOGIC("check access: %s", cls->descriptor);
	return !strstr(cls->descriptor, "Lcz/vutbr/policyenforcement/apkmon/");
}
    ...
    JValue* pResult)
{
	...
	if (allowReflectionAccess(declaringClass) && old_invokeNative)
		...
		LOG_TO_CAT("Block access of invokeNative on %s", declaringClass->descriptor);
}
...
static void Dalvik_java_lang_reflect_Field_setField(const u4* args, JValue* pResult)
{
	...
	if (allowReflectionAccess(declaringClass) && old_setField)
		...
		LOG_TO_CAT("Block access of setField on %s", declaringClass->descriptor);
}
...
static void Dalvik_java_lang_reflect_Field_setPrimitiveField(const u4* args, JValue* pResult)
{
	...
	if (allowReflectionAccess(declaringClass) && old_setPrimitiveField)
		...
		LOG_TO_CAT("Block access of setPrimitiveField on %s", declaringClass->descriptor);
}
