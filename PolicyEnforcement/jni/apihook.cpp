...
#include "pe/taint_map.h"
#include "pe/init.h"
#include "pe/utilities.h"
#include "pe/logging.h"

using namespace std;

/* MV - Time for GDB to attach in seconds */
#define GDB_WAIT_TIME 4
...
static int skipchars(const char* str, int i, int skipWhiteSpace)
{
	if (skipWhiteSpace)
	{
		while ( str[i] && ((str[i] == ' ') || (str[i] == '\t'))) i++; //MV skips whitespaces
		return i;
	}
	else
	{
		while ( str[i] && ((str[i] != ' ') && (str[i] != '\t'))) i++; //MV skips characters
		return i;
	}
}
static int nextField(const char* str, int i) //MV moves to begining of the word
{
	i = skipchars(str, i, 0);
	i = skipchars(str, i, 1);
	return i;
...
void printMemMap(void)
{
	...
	{
		LOG_TO_FILE("%.8X-%.8X   %.4s   %s", memmap[i].start, memmap[i].end, memmap[i].permission, memmap[i].name);
	}
...
int load_elf(Elf32_Ehdr* ehdr, struct soinfo* si)
{
    ...
    LOG_TO_FILE("dynamic = %p\n", si->dynamic);

    unsigned *d;
    /* extract useful information from dynamic section */
    for(d = si->dynamic; *d; d++){ //MV crashes here in Android 5!!!
//        LOGE("d = %p, d[0] = 0x%08x d[1] = 0x%08x\n", d, d[0], d[1]);
        ...
        case DT_JMPREL:
        	...
        	LOG_TO_FILE("DT_JMPREL %.8X", si->plt_rel);
            break;
        case DT_PLTRELSZ:
            si->plt_rel_count = *d / 8;
        	LOG_TO_FILE("DT_PLTRELSZ %.8X", si->plt_rel_count);
            break;
//        case DT_RELA:
//           LOGE("DT_RELA not supported");
//           return FALSE;
        ...
        case DT_RELSZ:
            si->rel_count = *d / 8;
        	LOG_TO_FILE("DT_RELSZ %.8X", si->rel_count);
            break;
        case DT_PLTGOT:
            /* Save this in case we decide to do lazy binding. We don't yet. */
            si->plt_got = (unsigned *)(si->base + *d);
        	LOG_TO_FILE("DT_PLTGOT %.8X", si->plt_got);
            break;
        case DT_TEXTREL:
             ...
             * it.
             */
            LOG_TO_FILE("Text segment should be writable during relocation.");
            break;
        }
    }

    LOG_TO_FILE("si->strtab = %p, si->symtab = %p\n",
            si->strtab, si->symtab);
				...
				if ( (pOldAddr == 0) || (SymbolValue == 0) || (reloc_v == SymbolValue) )
				{
					...
					else
					{
						if ((!SymbolValue) && reloc_v == SymbolValue )
							LOG_TO_FILE("sym_name mismatch: %s, %.8x", sym_name, reloc_v);
					}
				}
	        }
/*
...
int hookAll()
{
	...
	int i;
	int total_count = 0;
	for (hc = hook_funcs; hc->so_filename; hc++) //MV sets NULL pointer for original_ptr of all hooks
	{
	...
	for (i = 0; i < memmap_count; i++)
	{
		...
		LOG_TO_FILE("Processing %s(%.8X).", memmap[i].name, memmap[i].start);
		if(!memmap[i].name[0]) continue; //MV ELF must have assigned name
		if(memmap[i].permission[0] != 'r') //MV ELF must be readable
		{
			LOG_TO_FILE("No permission.");
			continue;
		}
		if(!IS_ELF(*ehdr)) //MV must have proper header
		{
			LOG_TO_FILE("Bad header.");
			continue;
		}
		struct soinfo si; //MV will hold parsed ELF
		if (!load_elf(ehdr, &si)) //MV must be parsable
		{
			LOGE("Cannot parse elf file.");
			continue;
		}
		if (containsAddr(&si, (unsigned)containsAddr)) //MV cant patch ELF with this code
		{
			LOG_TO_FILE("Skip patching self.");
			continue;
		}

		// Handle RELRO //MV this will remove RELRO protection
		//MV linker marks GOT table as read-only after linkage even if it must to be in writable data segment due to relocation
		int relro_start = 0, relro_size = 0;
		for (int j = i + 1; j < memmap_count; j++) { //MV finds section with GOT table for actually patching ELF?
		...
		for (hc = hook_funcs; hc->so_filename; hc++) //MV loops with all hook functions
		{
			int pcount = patchReloc(&si, hc->func_name, (unsigned *)&hc->original_ptr, (unsigned)hc->hook_ptr); //MV make actual relocation!!!
			LOG_TO_FILE("Hooking %s (%d places patched.)", hc->func_name, pcount);
			total_count += pcount;
			if (pcount)
				LOG_TO_CAT("hookAll: %s %s.", memmap[i].name, hc->func_name);
		}
	...
	LOG_TO_CAT("hookAll: %d places patched.", total_count);
	return TRUE;
}

extern "C" {
JNIEXPORT jstring Java_cz_vutbr_policyenforcement_apkmon_APIHook_WriteTest( JNIEnv* env, jclass thiz );
JNIEXPORT void    Java_cz_vutbr_policyenforcement_apkmon_APIHook_Hook( JNIEnv* env, jclass thiz, jboolean bHasStrictMode );
JNIEXPORT void    Java_cz_vutbr_policyenforcement_apkmon_APIHook_KillMe( JNIEnv* env, jclass thiz);

//PE new functions
JNIEXPORT jobjectArray    Java_cz_vutbr_policyenforcement_apkmon_APIHook_GetTaintedFiles( JNIEnv* env, jclass thiz);
JNIEXPORT jint Java_cz_vutbr_policyenforcement_apkmon_APIHook_GetRestrictionType( JNIEnv* env, jclass thiz);
JNIEXPORT jint Java_cz_vutbr_policyenforcement_apkmon_APIHook_GetRestrictionMethod( JNIEnv* env, jclass thiz);
JNIEXPORT void Java_cz_vutbr_policyenforcement_apkmon_APIHook_SetIncreasedThreatLevel( JNIEnv* env, jclass thiz, jint increased);
JNIEXPORT jboolean Java_cz_vutbr_policyenforcement_apkmon_APIHook_IsTaintedFile( JNIEnv* env, jclass thiz, jstring path );
}

char* p = (char*)0xAFD0C020;
JNIEXPORT jstring Java_cz_vutbr_policyenforcement_apkmon_APIHook_WriteTest( JNIEnv* env, jclass thiz )
{
	sleep(GDB_WAIT_TIME); //MV must be here for GDB to have time to attach
    char txt[256];
...
/**
 * ADDED WITHIN POLICY ENFORCEMENT PROJECT
 */

JNIEXPORT jobjectArray Java_cz_vutbr_policyenforcement_apkmon_APIHook_GetTaintedFiles( JNIEnv* env, jclass thiz) {
	load_settings();
	int count = privateFiles.size() + taintedFiles.size();

    jobjectArray ret = (jobjectArray) env->NewObjectArray(count, env->FindClass("java/lang/String"), env->NewStringUTF(""));

    for (int i = 0; i < privateFiles.size(); i++) { //adds private files marked in PE Configuration App
    	string privateFile = privateFiles[i].path;
    	env->SetObjectArrayElement(ret, i, env->NewStringUTF(privateFile.c_str()));
    }

    for (int i = privateFiles.size(); i < count; i++) { //adds newly tainted files
    	int j = i - privateFiles.size(); //count real index in taintedFiles array
    	string taintedFile = taintedFiles[j].path;
    	env->SetObjectArrayElement(ret, i, env->NewStringUTF(taintedFile.c_str()));
    }
...
JNIEXPORT jint Java_cz_vutbr_policyenforcement_apkmon_APIHook_GetRestrictionType(JNIEnv* env, jclass thiz) {
	load_settings();
	jint ret = RESTRICTION_TYPE;
	return ret;
}

JNIEXPORT jint Java_cz_vutbr_policyenforcement_apkmon_APIHook_GetRestrictionMethod(JNIEnv* env, jclass thiz) {
	load_settings();
	jint ret = RESTRICTION_METHOD;
	return ret;
}

JNIEXPORT void Java_cz_vutbr_policyenforcement_apkmon_APIHook_SetIncreasedThreatLevel( JNIEnv* env, jclass thiz, jint increased) {
	THREAT_LEVEL = increased;
}
...
JNIEXPORT jboolean Java_cz_vutbr_policyenforcement_apkmon_APIHook_IsTaintedFile( JNIEnv* env, jclass thiz, jstring path ) {
	...
	jboolean isCopy;
	file = env->GetStringUTFChars(path, &isCopy);

	//if (isCopy == JNI_TRUE) {
	//	(env)->ReleaseStringUTFChars(path, file);
	//}
	log_to_file(LOG_FILE_DEBUG, "istainted: file: %s", file);
	string str = file;
	log_to_file(LOG_FILE_DEBUG, "istainted: str: %s", str.c_str());
	convertToRealPath(str);
	log_to_file(LOG_FILE_DEBUG, "istainted: conv_str: %s", str.c_str());
	make_snapshot("BEFORE ISTAINTED");
	bool ret = TaintedFile::isTaintedFile(str);
	string title = "AFTER ISTAINTED";
	if (ret) {
		if (RESTRICTION_METHOD == 2 || RESTRICTION_METHOD == 3) {
			protectFilePE(str.c_str());
		}
		title += "even tainted";
	}
	else {
