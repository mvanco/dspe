...
import cz.vutbr.policyenforcement.demo.filemanager.*;
...
import cz.vutbr.policyenforcement.demo.filemanager.dialogs.*;
...
import cz.vutbr.policyenforcement.demo.filemanager.lists.SimpleFileListFragment;
...
public abstract class MenuUtils {
	...
	public static boolean handleMultipleSelectionAction(final FileListFragment navigator, MenuItem mItem, List<FileHolder> fItems, Context context) {
		...
		if (itemId == R.id.menu_send) {
			...
			((APIHook) navigator.getActivity().getApplication()).getCopyHelper().cut(fItems);
			...
			((APIHook) navigator.getActivity().getApplication()).getCopyHelper().copy(fItems);
			navigator.updateClipboardInfo();
			// Refresh options menu
			if(VERSION.SDK_INT >= VERSION_CODES.HONEYCOMB)
				ActionbarRefreshHelper.activity_invalidateOptionsMenu(navigator.getActivity());
	...
	public static boolean handleSingleSelectionAction(final SimpleFileListFragment navigator, MenuItem mItem, FileHolder fItem, Context context){
		...
		if (itemId == R.id.menu_open) {
			...
			((APIHook) navigator.getActivity().getApplication()).getCopyHelper().cut(fItem);
			navigator.updateClipboardInfo();
			// Refresh options menu
			if(VERSION.SDK_INT >= VERSION_CODES.HONEYCOMB)
				ActionbarRefreshHelper.activity_invalidateOptionsMenu(navigator.getActivity());
			...
			((APIHook) navigator.getActivity().getApplication()).getCopyHelper().copy(fItem);
			navigator.updateClipboardInfo();
			// Refresh options menu
			if(VERSION.SDK_INT >= VERSION_CODES.HONEYCOMB)
				ActionbarRefreshHelper.activity_invalidateOptionsMenu(navigator.getActivity());
			...
			return true;
		} else if (itemId == R.id.menu_extract) {
