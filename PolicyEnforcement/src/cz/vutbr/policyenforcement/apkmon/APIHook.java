package cz.vutbr.policyenforcement.apkmon;
...
import cz.vutbr.policyenforcement.apkmon.permissions.AurasiumPermission;
import cz.vutbr.policyenforcement.apkmon.permissions.ExecPermission;
import cz.vutbr.policyenforcement.apkmon.permissions.NativeLibraryPermission;
import cz.vutbr.policyenforcement.apkmon.permissions.PermissionRequestIntent;
import cz.vutbr.policyenforcement.apkmon.permissions.PrivacyPermission;
import cz.vutbr.policyenforcement.apkmon.permissions.SMSPermission;
import cz.vutbr.policyenforcement.apkmon.permissions.SocketConnectPermission;
import cz.vutbr.policyenforcement.demo.filemanager.util.CopyHelper;

class AppendLog {
		
	static File logFile = new File("/sdcard/pe_log_java.txt");
...
class TransactionHandlerFactory {
	/*
	 * Helper function to convert a code to its string representation as in its
	 * defining class
	 */
	...
	public static TransactionHandler getHandler(String Descriptor, int Code) {
		...
		if (Descriptor.equals("android.content.IContentProvider") && TransactionName.equals("QUERY_TRANSACTION")) {
			return new PEHandler();
		}
...
class TransactionHandler {
	/*
	 * Security checks before the transaction happens, useful to modify its
	 * arguments Note that the first interface token is not included in the
	 * parcel and is handled separately by underlying native code. Return
	 * values: 0 - Nothing is changed, the API hooking trampoline should proceed
	 * normally. byte[] - Marshalled modified Parcel should be injected back to
	 * the ioctl command buffer.
	 */
	...
	/*
	 * Security checks after the transaction happens, useful to modify its
	 * results Return values: NULL - Nothing is changed, the API hooking
	 * trampoline should proceed normally. byte[] - Marshalled modified Parcel
	 * should be injected back to the ioctl reply buffer.
	 */
...
class SMSSendTextHandler extends TransactionHandler {
	...
	static {
		...
		try {
			...
			iSmsCls.getMethod("sendText", new Class[] { String.class, String.class, String.class, String.class,
	...
	public byte[] HandleBeforeTransact(String Descriptor, Parcel parcel) {
		...
		APIHook.LOG_I("HandleBeforeTransact",
				String.format("SendText: dest:%s, src:%s, text:%s", destAddr, scAddr, text));
	...
	public byte[] HandleAfterTransact(Parcel parcel) {
		if (intercepted) {
			try {
				...
				// This is very likely to be caused by our intercepting attempt
				// (Replacing destAddr to ""),
				// And we don't want the resultant exception to be visible by
...
class BroadcastReceiverHandler extends TransactionHandler {
	...
	public byte[] HandleBeforeTransact(String Descriptor, Parcel parcel) {
		...
		APIHook.LOG_I("HandleBeforeTransact",
				String.format("Broadcast Receiver %s %s will be called", IntentAction, IntentReceiver));
		return null;
	}
}
...
class PEHandler extends TransactionHandler {
	...
	private void readArguments(Parcel parcel) {
		int length = 0;
		
		if (android.os.Build.VERSION.SDK_INT >= 18) {
			callingPkg = parcel.readString();
		}
		url = Uri.CREATOR.createFromParcel(parcel);
		
		
		projection = parcel.createStringArray();
		selection = parcel.readString();
		selectionArgs = parcel.createStringArray();
		sortOrder = parcel.readString();
		binder = parcel.readStrongBinder();
		String s = "stop here";
	}
	
	public static void logParcelContent(Parcel parcel) {
		...
		for (int i = 0; i < result.length; i++) {
			...
			String c_int = Integer.toHexString(ord); //two characters representing hexa format
			if (c_int.length() == 1) {
				c_int = "0" + c_int;
			}
	...
	private boolean isProtectedFile() {
		String[] taintedFiles = APIHook.GetTaintedFiles(); //TODO: rename to GetTaintedFiles()
		for (String taintedFile : taintedFiles) {
			if (url.toString().contains(taintedFile)) {
				return true;
			}
		}
		return false;
	...
	public byte[] HandleBeforeTransact(String Descriptor, Parcel parcel) {
		...
		readArguments(parcel);
		logParcelContent(parcel);
		String path = Utility.extractPathFromUri(url);
		if (APIHook.IsTaintedFile(path)) {
			if (APIHook.GetRestrictionMethod() == 1 || APIHook.GetRestrictionMethod() == 3) { // communication mediation, both
				if (APIHook.GetRestrictionType() == 1) { //Dialog
					if (!Utility.PolicyCheck(new PrivacyPermission(PrivacyPermission.PRIVACY_TYPE.Sharing))) { // deny
						...
						/*
						// MODIFICATION TO OTHER FILE (GMAIL APPLICATIONS CRASHES :(
						Parcel p = Parcel.obtain();
						p.writeNoException();
						
						if (android.os.Build.VERSION.SDK_INT >= 18) {
							p.writeString("callingPkg"); // not tested! using older version
						}
						Uri fakeUri = Uri.parse("content://cz.vutbr.policyenforcement.demo.filemanager/storage/sdcard0/!test/fake.txt");
						fakeUri.writeToParcel(p, 0);
						int length = 0;
						if (projection != null) {
				...
				else if (APIHook.GetRestrictionType() == 2) { //Applicaton Termination
					intercept = true; //always intercepting
...
class LocationAddProximityAlertHandler extends TransactionHandler {
	...
	public byte[] HandleBeforeTransact(String Descriptor, Parcel parcel) {
		if (!Utility.PolicyCheck(new PrivacyPermission(PrivacyPermission.PRIVACY_TYPE.GPS_Proximity))) {
			...
			p.writeFloat(radius);
			p.writeLong(0); // Set expiration to zero to render the request
							// useless
			if (intent != null) {
				p.writeInt(1);
...
class LocationRequestUpdatesHandler extends TransactionHandler {
	...
	public byte[] HandleBeforeTransact(String Descriptor, Parcel data) {
		if (!Utility.PolicyCheck(new PrivacyPermission(PrivacyPermission.PRIVACY_TYPE.GPS_Last))) {
			...
			// _arg5 =
			// android.location.ILocationListener.Stub.asInterface(data.readStrongBinder());
	...
	public byte[] HandleAfterTransact(Parcel parcel) {
		if (intercept) {
			// Inhibit the server side null pointer exception as we just passed
...
public class APIHook extends Application {
	static final boolean APPLIED_HOOKS = true;
	static final boolean DEBUG = true;

	public static native String WriteTest();
	public static native void KillMe();
	public static native void Hook(boolean HasStrictMode);
	
	public static native String[] GetTaintedFiles();
	public static native int GetRestrictionType();
	public static native int GetRestrictionMethod();
	public static native void SetIncreasedThreatLevel(int increased);
	public static native boolean IsTaintedFile(String path);

	public static native void saveDecisions(String path, byte[] data);
	public static native byte[] loadDecisions(String path);

	static {
		int i = 5; // MV just for breakpoint
		// android.os.Debug.waitForDebugger(); //MV waiting for debugger to
		// attach so that immediate breakpoints in native code are functionsl
		System.loadLibrary("apihook");
		/*
		 * android.os.Build.VERSION_CODES.GINGERBREAD == 0x9, when StrictMode is
		 * introduced
		 */
		if (APPLIED_HOOKS) {
			...
			for (int j = 0; j < 100; j++) {
				Log.e("PolicyEnforcement", "********* HOOKS ARE NOT APPLIED!!!! *************");
			}
		}

	}

	/** <From FileManagerApplication> */
	...
	private static byte[] ProcessTransaction(Boolean isRemote, int ThreadID, String Descriptor, int transactionCode,
			Parcel data) {
		// String TransactionName =
		// TransactionHandlerFactory.TranslateCode(Descriptor, transactionCode);
		// APIHook.LOG_I("apihook", String.format("%d %s %s %s", ThreadID,
		// isRemote ? "on_BC_TRANSACTION" : "on_BR_TRANSACTION",
	...
	private static byte[] ProcessReply(Boolean isRemote, int ThreadID, Parcel data) {
		//PEHandler.logParcelContent(data);
		...
		// APIHook.LOG_I("apihook", String.format("%d %s %s", ThreadID, isRemote
		// ? "on_BR_REPLY" : "on_BC_REPLY", tr.Descriptor));

		String TransactionName = TransactionHandlerFactory.TranslateCode(tr.Descriptor, tr.Code);
	...
	/*
	 * Callback function for Bx_TRANSACTION/REPLY, take care when dealing with
	 * remote/local objects in the parcel Return values: 0 - Nothing is changed,
	 * the API hooking trampoline should proceed normally. byte[] - Marshaled
	 * modified Parcel should be injected back to the ioctl command buffer.
	 * 
	 * BC_TRANSACTION/BR_REPLY pair indicates a remote call and its return
	 * value, while BR_TRANSACTION/BC_REPLY pair indicates a request to call
	 * local method and command to send the call's results to remote side.
	 */
	// TODO: Remove redundant command argument
	public static byte[] on_BC_TRANSACTION(int ThreadID, int command, int transactionCode, String Descriptor,
			Parcel parcel) {
		if (!StartLogged) {
			Utility.LogAccess("*****AppStart*****", null);
			StartLogged = true;
		}
		APIHook.LOG_I("MV", String.format("%s: BC_TRANSACTION", Descriptor));
		return ProcessTransaction(true, ThreadID, Descriptor, transactionCode, parcel);
	}

	public static byte[] on_BR_TRANSACTION(int ThreadID, int command, int transactionCode, String Descriptor,
			Parcel parcel) {
	...
	public static int onBeforeConnect(int sockfd, String addr, int port) {
		String RemoteIP = (addr.startsWith("::ffff:") ? addr.substring(7) : addr); // IPv4-mapped
																					// address.
	...
	public static int onDNSResolve(String DomainName, String IPaddr) {
		// Sometimes this function gets called with DomainName(in ip form) ==
		// addr
...
class Utility {
	static final String ContentUriProtocol = "content://";
	...
	static final String SharingDialogActivityName = SharingDialogActivity.class.getName();
	static final ComponentName LocalConsentDialogName = new ComponentName(AppPackageName, ConsentDialogActivityName);
	static final ComponentName SharingDialogName = new ComponentName(AppPackageName, SharingDialogActivityName);
	...
	static final Intent SecurityManagerIntent = new Intent().setClassName(SecurityManagerPackage,
			SecurittManagerService);
	...
	private static boolean isASMAvailable() {
		...
		Future<Boolean> result = executor.submit(new Callable<Boolean>() {
			public Boolean call() throws Exception {
				...
				// APIHook.getSystemContext().getPackageManager().resolveService(null,
				// 0);
				return APIHook.getSystemContext().getPackageManager().resolveService(SecurityManagerIntent,
	...
	public static synchronized void LogAccess(String PolicyName, String PolicyUID) {
		if (!APIHook.DEBUG)
			return;
		try {
			BufferedWriter out = new BufferedWriter(new FileWriter("/sdcard/ssaccess.txt", true));
			out.write(String.format("%s %s: %s %s\n", now("yyyy-MM-dd hh:mm:ss"),
					APIHook.getSystemContext().getPackageName(), PolicyName, PolicyUID != null ? PolicyUID : ""));
			out.close();
		} catch (Exception e) {
		}
	}
	...
	// Retrieve saved policy decisions, 1 for allow, 0 for deny, -1 for kill
	// app, -2 for non-existent.
	...
	private static void SavePolicyDecision(String PolicyName, String PolicyUID, int decision) {
	...
	/*
	 * Perform a policy check of the given operation. Delegates to ASM if it is
	 * installed on the device. Otherwise use local logic: Check for
	 * saved/remembered decision first, then ask for user consent. returns: true
	 * for allow, false for deny.
	 */
	public static synchronized boolean PolicyCheck(AurasiumPermission permission) {
		int decision = 0;
		if (isASMAvailable()) {
			RemotePermissionChecker checker = new RemotePermissionChecker(permission,
					SecurityManagerIntent.getComponent(), RemotePermissionChecker.RemoteType.Service);
			...
			{
				RemotePermissionChecker checker = null;
				if (permission.getGroupingIdentifier() == "privacy") {
					checker = new RemotePermissionChecker(permission, SharingDialogName,
						RemotePermissionChecker.RemoteType.Activity);
				}
				else {
					checker = new RemotePermissionChecker(permission, LocalConsentDialogName,
							RemotePermissionChecker.RemoteType.Activity);
				}
		...
		if (decision == -1) {
			APIHook.LOG_I(APIHook.getSystemContext().getPackageName(), "Application termination at user's request.");
			APIHook.KillMe();
		}
		return decision == 1;
	}
		...
		try {
			...
			if (AM_startActivity == null) {
				...
				if (android.os.Build.VERSION.SDK_INT < 14)
					AM_startActivity = IfAM
							.getMethod("startActivity",
									new Class[] { cls_IApplicationThread, Intent.class, String.class, Uri[].class,
											int.class, IBinder.class, String.class, int.class, boolean.class,
											boolean.class });
				else if (android.os.Build.VERSION.SDK_INT < 16) // This is the
																// signature for
																// ICS
				{
					pfd = Class.forName("android.os.ParcelFileDescriptor");
					AM_startActivity = IfAM.getMethod("startActivity",
							new Class[] { cls_IApplicationThread, Intent.class, String.class, Uri[].class, int.class,
									IBinder.class, String.class, int.class, boolean.class, boolean.class, String.class,
									pfd, boolean.class });
				} else if (android.os.Build.VERSION.SDK_INT < 18) { // Signature
																	// for
																	// Jellybean
																	// 4.1, 4.2
					pfd = Class.forName("android.os.ParcelFileDescriptor");
					AM_startActivity = IfAM.getMethod("startActivity",
							new Class[] { cls_IApplicationThread, Intent.class, String.class, IBinder.class,
									String.class, int.class, int.class, String.class, pfd, Bundle.class });
				} else { // Jellybean 4.3 or later
					pfd = Class.forName("android.os.ParcelFileDescriptor");
					AM_startActivity = IfAM.getMethod("startActivity",
							new Class[] { cls_IApplicationThread, String.class, Intent.class, String.class,
									IBinder.class, String.class, int.class, int.class, String.class, pfd,
									Bundle.class });
			...
			if (android.os.Build.VERSION.SDK_INT < 16)
				AM_startActivity.invoke(mAm, null, intent, intent.getType(), null, 0, null, null, 0, false, false, null,
						null, false);
			if (android.os.Build.VERSION.SDK_INT < 18)
				AM_startActivity.invoke(mAm, null, intent, intent.getType(), null, null, 0, 0, null, null, null);
			else
	...
	public static boolean LowlevelStartService(Intent intent) {
		try {
			...
			if (AM_startService == null) {
				AM_startService = IfAM.getMethod("startService",
						new Class[] { cls_IApplicationThread, Intent.class, String.class });
			}
			Object mAm = AM_getDefault.invoke(null, (Object[]) null);
	...
	public static Uri extractUri(Parcel parcel) {
		byte[] result = parcel.marshall();
		...
		Pattern p = Pattern.compile("content://(\\p{Alnum}|\\p{Punct})*?(\\p{Space}|\\x00)");
	...
	public static String extractPathFromUri(Uri uri) {
		...
		//"^content://(\\p{Alnum}|[\\p{Punct}&&[^/]])*?/(\\p{Alnum}|\\p{Punct})*?$"
...
class RemotePermissionChecker implements Runnable {
	...
	public RemotePermissionChecker(AurasiumPermission permission, ComponentName target, RemoteType type) {
