...
#           print "tagtype = ", _type
            ...
            if _type == START_DOCUMENT :
                ...
                # FIXME : use namespace
                if self.xmlns == False :
                    for i in range(self.axml.getNamespaceCount()):
                        self.buff += "xmlns:%s=\"%s\"\n" % ( self.axml.getNamespacePrefix( i ), self.axml.getNamespaceUri( i ) )
                        self.xmlns = True
    ...
    def getAttributeValue(self, index) :
        ...
        if _type == TYPE_STRING :
            return self.axml.getAttributeValue( index ).replace('\\', '\\\\')\
                   .replace('\n', '\\n')\
                   .replace('&', '&amp;')\
                   .replace('"', '&quot;')\
                   .replace('<', '&lt;')\
            ...
            return "?%s0x%08X" % (self.getPackage(_data), _data)
