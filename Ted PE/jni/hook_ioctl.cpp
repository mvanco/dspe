...
void make_log(char *command, unsigned write_buffer) {
	...
	int TokenLen = 0;
	getInterfaceToken((unsigned)tr->data.ptr.buffer, tr->data_size, InterfaceName, TokenLen);
	JNIEnv* env = JNIHelper::Self().GetEnv();
	jstring jIfName = env->NewStringUTF(InterfaceName);
	const char* desc = env->GetStringUTFChars(jIfName, 0);
	char bbb[50];
	snprintf(bbb, 50, "%s: %s", desc, command);
	__android_log_print(ANDROID_LOG_INFO,"MVcpp", "%s", bbb);
}
...
//    }
    ...
    if ((tr->flags & TF_STATUS_CODE) == 0)
    {
		...
		if (getInterfaceToken((unsigned)tr->data.ptr.buffer, tr->data_size, InterfaceName, TokenLen))
		{
			...
			if (cmd == BC_TRANSACTION) {
				snprintf(bbb, 100, "%s: BC_TRANSACTION", desc);
			}
			else if (cmd == BR_TRANSACTION) {
				snprintf(bbb, 100, "%s: BR_TRANSACTION", desc);
			}
			__android_log_print(ANDROID_LOG_INFO,"MVcpp", "%s", bbb);
			*/
    ...
    else
    {
    	...
    	LOG_TO_FILE("parse_transaction_data: status: %.8x", err);
    }
    return rtnPtr;
}
...
char* process_reply_data(unsigned read_buffer, size_t* oldDataSize)
{
    ...
    if ((tr->flags & TF_STATUS_CODE) == 0)
    {
		...
		char bbb[100];
		if (cmd == BC_REPLY) {
			snprintf(bbb, 100, "%s: BC_REPLY", "unknown");
		}
		else if (cmd == BR_REPLY) {
			snprintf(bbb, 100, "%s: BR_REPLY", "unknown");
		}
		__android_log_print(ANDROID_LOG_INFO,"MVcpp", "%s", bbb);
		*/
...
//			env->DeleteGlobalRef(parcel);
    ...
    {
    	unsigned  err = *(unsigned*)(tr->data.ptr.buffer);
    	LOG_TO_FILE("process_reply_data: status: %.8x", err);
    }
    return rtnPtr;
}
...
//		LOGIC("Thread(%d): %d %s", gettid(), cid, cmd_name(cmd));
		switch(cmd){
		case BC_TRANSACTION:
			//make_log("BC_TRANSACTION", write_buffer);
		case BC_REPLY:
			//make_log("BC_REPLY", write_buffer);
			transCmd = cmd;
			if (cmd == BC_TRANSACTION)
			...
			if (newBufPtr)
			{
				LOG_TO_FILE("Replacing buffer(%d) with %.8x", oldSize, newBufPtr);
...
//			LOGIC("Thread(%d): %d %s", gettid(),uid, reply_name(cmd));
			...
			case BR_FAILED_REPLY:
				...
				break;
			case BR_TRANSACTION:
				//make_log("BR_TRANSACTION", read_buffer);
			case BR_REPLY:
				//make_log("BR_REPLY", read_buffer);
				transReply = cmd;
				...
				if (newBufPtr)
				{
					LOG_TO_FILE("Replacing buffer(%d) with LEAKY %.8x", oldSize, newBufPtr);
					allocBufs[allocBufCount++] = newBufPtr;
				}
...
int my_libc_ioctl(int fd, int request, ...)
{
	...
	//return ioctl(fd, request, arg); //MV this makes default ioctl function without mediation
