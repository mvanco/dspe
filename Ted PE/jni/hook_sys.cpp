...
extern "C" {
JNIEXPORT void    Java_fr_xgouchet_apkmon_APIHook_saveDecisions( JNIEnv* env, jobject thiz, jstring path, jbyteArray data);
JNIEXPORT jbyteArray Java_fr_xgouchet_apkmon_APIHook_APIHook_loadDecisions( JNIEnv* env, jobject thiz, jstring path);
}

#ifndef NDEBUG
#define LOG_FILE "/sdcard/syslog.txt"
#endif
...
void *my_libc_dlopen(const char *filename, int flag)
{
	...
	//MLOG("dlopen %s, filename");
	LOG_TO_CAT("dlopen  %s", filename);
	LOG_TO_FILE("dlopen  %s", filename);
...
char *get_file_name(int fd) {
	...
	int result = readlink(pathfile,filename_buffer,sizeof(filename_buffer));
	if (result == -1) {
		memset(filename_buffer,'\0',MY_BUFFER_SIZE);
	}

	if (!startsWith("pipe:", filename_buffer) && !startsWith("/dev/cpuctl/apps/tasks", filename_buffer))
...
int get_file_info_from_fd(int fd) {
	...
	//LOGIC("MV get_file_info_from_fd, filename: %s", filename_buffer);
	return result;
}
...
pid_t my_libc_fork(void)
{
	...
	else //we are the parent process, need to wait for the filepath to come out of the pipe.
	{
		...
		LOG_TO_FILE("fork: filename = %s, %.8x", path, jstr_filename);
		int allow = env->CallStaticIntMethod(JNIHelper::Self().gApiHookMetadata.Clazz,
				JNIHelper::Self().gApiHookMetadata.onBeforeExecvp,
				jstr_filename);
		LOG_TO_FILE("fork: allow = %d", allow);
...
int my_libc_execvp(const char *filename, char *const argv [])
{
	...
	if (allow)
	{
		LOG_TO_FILE("execvp: allow %s %x", filename, argv);
		return execvp(filename, argv);
	}
	else
	{
		LOG_TO_FILE("execvp: denied %s %x", filename, argv);
		errno = -EACCES;
		return -1;
	}
}
...
JNIEXPORT void Java_cz_vutbr_policyenforcement__apkmon_APIHook_saveDecisions( JNIEnv* env, jobject thiz, jstring path, jbyteArray data )
{
	char file[1024];
	...
	if (f)
	{
		...
		LOG_TO_CAT("Saving decisions to %s failed.", AurasiumPolicyFile);
}

JNIEXPORT jbyteArray Java_fr_xgouchet_apkmon_APIHook_loadDecisions( JNIEnv* env, jobject thiz, jstring path)
{
	char file[1024];
	const char *path_ptr = env->GetStringUTFChars(path, NULL);
	snprintf(file, sizeof(file), "%s/"AurasiumPolicyFile, path_ptr);
...
FILE  *my_libc_fopen(const char *file, const char *mode) {
	LOG_TO_CAT("fopen, file: %s, mode: %d", file, mode);
	LOG_TO_FILE("fopen, file: %s, mode: %d", file, mode);
...
int my_libc_open(const char *file, int flags, int mode) {
	...
	handle_before_open(file, flags, mode);
	result = open(file,flags,mode); //MV interconnection with our hooks
	handle_after_open(result, file, flags, mode);
	LOGE("open returns %d",result); //MV interconnection with our hooks
	return result;
}
...
int my_libc_close(int fd)
{
	...
	handle_before_close(fd); //MV interconnection with our hooks
	int result = close(fd);
	handle_after_close(result, fd); //MV interconnection with our hooks
...
int my_libc_write(int file, const void *buffer, size_t count) {
	...
	handle_before_write(file, buffer, count); //MV interconnection with our hooks
	int result = write(file,buffer,count);
	handle_after_write(result, file, buffer, count); //MV interconnection with our hooks
...
int my_libc_read(int file, void *buffer, size_t length) {
	...
	handle_before_read(file, buffer, length); //MV interconnection with our hooks
	int result = read(file,buffer,length); //MV interconnection with our hooks
	handle_after_read(result, file, buffer, length); //MV interconnection with our hooks
